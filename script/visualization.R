...
graph = function(data, string, filename){
  ...
  names(result) = c('산업경제', '환경안전', '정책거버넌스', '외교안보', '단어 비율', '기사 숫자', 'time')
  ...
  ggsave(paste0('result/', filename, '.jpg'), width = 30, height=18, units="cm")
}

graph(data,"한수원|산자부|원자력문화재단|원자력안전위원회", '원자력계 전반')
graph(data,"한수원", '한수원')
graph(data,"산자부", '산자부')
graph(data,"원자력문화재단", '원자력문화재단')
graph(data,"원자력안전위원회", '원자력안전위원회')
graph(data,"", '전체')
string = ("경향신문|국민일보|내일신문|동아일보|문화일보|서울신문|세계일보|조선일보|중앙일보|한겨레|한국일보|
...
강원일보|매일신문|부산일보")
graph(data, string, '언론사 전체')
graph(data, "경향신문|국민일보|내일신문|동아일보|문화일보|서울신문|세계일보|조선일보|중앙일보|한겨레|한국일보", "종합일간지")
graph(data, "뉴스1|뉴시스|연합뉴스|연합뉴스TV", '뉴스통신사')
graph(data, "채널A|한국경제TV|JTBC|KBS|MBC|MBN|SBS|TV조선|YTN", '방송사')
graph(data, "매일경제|머니투데이|서울경제|아시아경제|이데일리|조선비즈|파이낸셜뉴스|한국경제|헤럴드경제|SBS CNBC", "경제지")
graph(data, "노컷뉴스|데일리안|미디어오늘|오마이뉴스|쿠키뉴스|프레시안", "인터넷 언론")
graph(data, "디지털데일리|디지털타임스|블로터|아이뉴스24|전자신문", "IT디지털")
graph(data, "매경이코노미|머니S|시사IN|신동아|이코노미스트|일다|주간경향|주간동아|주간조선|주간한국|중앙SUNDAY|팝뉴스|한겨레21|한경비즈니스", "주간월간지")
graph(data, "강원일보|매일신문|부산일보", "지역언론")

graph(data, "경향신문|한겨레", "진보")
graph(data, "조선일보|동아일보", "보수")
